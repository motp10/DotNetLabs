...
{
    public override string TokenName => "show";
    ...
    public override ParseResultType TryParse(ICommandBuilder commandBuilder, IEnumerator<string> enumerator)
    {
        if (enumerator.Current == TokenName)
        {
            if (enumerator.MoveNext())
            {
                ...
                if (NextArgument != null) return NextArgumentParse(new FileShowBuilder(), enumerator);
                if (NextFlag != null) return NextFlagParse(new FileShowBuilder(), enumerator);
