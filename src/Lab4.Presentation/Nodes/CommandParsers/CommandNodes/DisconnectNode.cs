...
{
    public override string TokenName => "disconnect";
    ...
    public override ParseResultType TryParse(ICommandBuilder commandBuilder, IEnumerator<string> enumerator)
    {
        if (enumerator.Current == TokenName)
        {
            if (enumerator.MoveNext())
            {
                ...
                if (NextArgument != null) return NextArgumentParse(new DisconnectBuilder(), enumerator);
                if (NextFlag != null) return NextFlagParse(new DisconnectBuilder(), enumerator);
            }
